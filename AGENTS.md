...
## Next Steps (Phase 2.11 Ready)

### Что было сделано (Phase 2.11)

1. ✅ Class lowering реализован — `Stmt::Class` в `lower_module()`
2. ✅ Struct lowering реализован — `Stmt::Struct` в `lower_module()`
3. ✅ `__init__` автовызов — при `Counter(10)` вызывается `Counter_init`
4. ✅ Methods — определяются и вызываются, `self` добавляется автоматически
5. ✅ `lower_method` использует `Linkage::Import` (не Export) — избегает double declare
6. ✅ Дедупликация `self` — методы с уже указанным `self` не получают второй параметр
7. ✅ `test_class.py` компилируется и выводит `11`
8. ✅ Все тесты проходят (6 cranelift + 45 front)
9. ✅ `cargo clippy` чистый — 0 warnings

### Что надо сделать (Phase 3 — Performance)
...
1. **Escape analysis** — stack allocation для объектов, не выходящих за scope функции
2. **Allocation hoisting** — вынести alloc за пределы loops
3. **Auto-free passes** — автоматическое освобождение памяти

#### 🟢 LOW PRIORITY

1. **Custom Lock/Spawn ISLE rules** — когда concurrency будет реализовано
2. **Coroutine lowering** — async/await

### Следующий этап — Phase 4 (Concurrency)
