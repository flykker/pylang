...
**Phase 2.13 — Code Review Cleanup** в процессе (см. PLAN.md)
...
## Next Steps (Phase 2.13 — Code Review Cleanup → Phase 3)

### Phase 2.13 — Code Review Cleanup (ТЕКУЩИЙ ЭТАП)

#### 🔴 HIGH PRIORITY — Critical Bugs

1. **i64::MIN UB в runtime** (`runtime/src/lib.rs:74`) — `(-x) as usize` крашится
2. **Type::Unit → I64** (`lower.rs:512`) — нарушение Type Mapping Consistency
3. **ast_type_to_clif swallows error** (`lower.rs:522-527`) — silent fallback на I64

#### 🟡 MEDIUM PRIORITY — Code Quality

1. **Slice stub** — игнорирует start/end/step
2. **clone() хеш-таблиц** — O(N×M) overhead на каждый вызов функции
3. **Dead code** — 6 SemaError variants, collect_identifiers_stmts, _dummy_slot, _main_fn
4. **check_for target** — должен выводить тип из iterable, не hardcoded I64
5. **field_type не проверяет struct** — только class
6. **parser discriminant** — std::mem::discriminant не проверяет payload
7. **builtins stubs** — range(), str(), input() молча возвращают неверные значения
8. **heap allocator** — не thread-safe

#### 🟢 LOW PRIORITY

1. **Тесты** — cargo test = 0 passed, нужно добавить
2. **#[allow(clippy::*)]** — реализовать Default вместо подавления
3. **dealloc no-op** — задокументировать
4. **--emit ir** — заглушка в CLI
5. **bare raise** — не поддерживается в parser

### После Phase 2.13 — Phase 3 (Performance)
