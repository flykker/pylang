...
**Phase 2.9 — Refactor & Harden (Code Review Fixes)** 🔄 IN PROGRESS
...
- 🔄 **Clippy warnings** — 5 warnings исправляются
- 🔄 **Documentation sync** — PLAN.md/AGENTS.md синхронизируются с кодом
...
### 🔴 CRITICAL: Class и Struct НЕ имеют lowering!
...
## Next Steps (Phase 2.9 Ready)

### Что было сделано (Phase 2.8)
...
6. ✅ Parser bug исправлен — тело функции парсится правильно
7. ✅ Break/Continue lowering добавлен
8. ✅ Тесты: 46 проходят
9. ✅ Parser: column-based indentation tracking в parse_suite()
   - Тело while/if/for теперь корректно ограничивается отступами
10. ✅ Lowering: убран hardcoded "x" из lower_while
    - while теперь работает с любой переменной
11. ✅ Lowering: убран after_block из lower_for
    - Исправлен verifier error в for loop
12. ✅ Lowering: exit_block не seal'ится раньше времени
    - Переменные после while/for loop теперь корректны
    - break/continue работают правильно

### Что надо сделать (Phase 2.9 — Code Review Fixes)

#### 🔴 HIGH PRIORITY

1. **Stub lowering** — match/try/raise/yield/with в lower.rs должны возвращать explicit Err вместо молчащей неправильной логики
2. **Type consistency** — Expr::Char I32→I64

#### 🟡 MEDIUM PRIORITY

1. **Clippy warnings** — 5 warnings (next/default/empty_loop/identical_if)
2. **Documentation sync** — PLAN.md/AGENTS.md таблицы соответствуют коду

#### Следующий этап - Phase 3

После Phase 2.9:
1. Class/Struct lowering
2. Lambda lowering
3. For (без range) lowering
4. Escape analysis (stack allocation)
5. Custom Lock/Spawn ISLE rules
6. Allocation hoisting, auto-free passes
