...
**Phase 2.15 — F-strings** завершена ✅
  - ✅ `f"text {expr} text"` — lexer detects `/f/` prefix + `"`, reads segments
  - ✅ `FString(Vec<FStringSeg>)` токен с `Lit(String)` / `Expr(String)` частями
  - ✅ `Expr::FString(Vec<FStringPart>)` AST узел
  - ✅ Sema: проверяет sub-expression'ы, возвращает `str`
  - ✅ Lowering `print(f"..." )`: выводит литералы через `print_str`, выражения через `print_int`
  - ✅ Lowering (general): `alloc` буфер + копирование литералов, `int_to_str` для выражений
  - ✅ Runtime: `int_to_str(buf, i64) → i64` — конвертация числа в строку (без memcpy/alloc)
  - ✅ 59 tests, clippy clean
  - ✅ `f"Running on port {port} ...\n"` в test.py работает (выводит "Running on port 8080 ...")
**Phase 2.16 — Class Field Registration Fix** завершена ✅
  - ✅ Исправлен segfault при `self.d[key] = val` в методах класса
  - ✅ `Stmt::Assign(Ident)` в теле класса регистрирует поле
  - ✅ `__init__` body сканируется на `self.field =` → поля регистрируются
  - ✅ test.py (Router класс с dict subscript assign) — работает без краша
  - ✅ 59 tests, clippy clean
**Phase 2.17 — Closure ABI Fix** завершена ✅
  - ✅ Исправлен баг: closure function имел лишний `__closure_*` param (дублирующий `closure_ptr`)
  - ✅ Root cause: `hoisted_params` в `lower_fn` добавлял `__closure_X`, дублирующий `closure_ptr` от `lower_fn_closure`
  - ✅ ABI mismatch: функция ожидала 3 params `[closure_ptr, __closure_X, func]`, call site передавал 2 args
  - ✅ Параметр `func` получал мусор (`8`) → `dict_set` хранил неверное значение
  - ✅ Fix: убрал `__closure_*` из `hoisted_params`, `lower_fn_closure` сам добавляет `closure_ptr`
  - ✅ Decorator factory `@router.post("/health")` → `router.routers["/health"]()` работает
  - ✅ 59 tests, clippy clean
**Phase 2.18 — Type Annotations Required + Type Propagation** завершена ✅
  - ✅ `Stmt::AnnAssign` для `name: Type = expr` (class-level field declarations)
  - ✅ Generic types: `dict[str, Callable]`, `list[int]` (парсинг уже был)
  - ✅ Full type annotations required: params, let, class fields, func return (опционально)
  - ✅ ClassDef фикс: `methods` и `fields` теперь корректно заполняются
  - ✅ `field_type` теперь возвращает тип поля класса (раньше всегда `Unit`)
  - ✅ `element_type` теперь обрабатывает `dict[K, V]` → V, `list[T]` → T
  - ✅ `call_return_type` теперь обрабатывает конструкторы классов (`Class(name)` → `Class(name)`)
  - ✅ Decorator map tracing: `self.dict["/health"]()` → return type из декоратора
  - ✅ Self param type: `self` в методах получает `Type::Class(name)`
  - ✅ `types_equal` нормализует `Named("str")` ↔ `String`, `Named("dict")` ↔ `Generic{dict,...}`
  - ✅ route_fn_ret_types hack удалён полностью из lowering
  - ✅ `Stmt::Let` теперь корректно записывает в глобальные переменные
  - ✅ `print(identified_string_var)` — теперь использует `print_str`
  - ✅ 52 tests (+3), clippy clean
  - ✅ test.py компилируется и запускается (HTTP сервер)
**Phase 2.19 — Segfault Fix + String Variable Propagation + F-string CFG Fix** завершена ✅
  - ✅ Исправлен segfault: `block_filled` break, `_dummy_slot` (32,16) alignment, `call_fn`/`dict_call` trampolines
  - ✅ Исправлен `collect_string_vars`: рекурсивный обход `if`/`while`/`for`/`loop`/`match` + проверка type annotations
  - ✅ Убран `append_str_or_int` из `lower_fstring` — ломал Cranelift CFG (sealed merge_block в середине цикла)
  - ✅ `content: str = self.routers["/health"]()` → `{content}` в f-string теперь корректно копирует строку
  - ✅ 59 tests, clippy clean, HTTP сервер: `{'health':'ok'}` в body
**Phase 2.20 — Code Review & Refactor** запланирована ⬜
...
## Next Steps (Phase 2.14 ✅ → 2.15 ✅ → 2.16 ✅ → 2.17 ✅ → 2.18 ✅ → 2.19 ✅ → Phase 3)

### Phase 2.15 — F-strings ✅
- ✅ `f"text {expr} text"` — lexer detects `/f/` prefix + `"`, reads segments
- ✅ `FString(Vec<FStringSeg>)` токен с `Lit(String)` / `Expr(String)` частями
- ✅ `Expr::FString(Vec<FStringPart>)` AST узел
- ✅ Sema: проверяет sub-expression'ы, возвращает `str`
...
### Phase 2.16 — Class Field Registration Fix ✅
- ✅ Исправлен segfault при `self.d[key] = val` в методах класса
- ✅ `Stmt::Assign(Ident)` в теле класса регистрирует поле
- ✅ `__init__` body сканируется на `self.field =` → поля регистрируются
- ✅ test.py (Router класс с dict subscript assign) — работает без краша
...
### Phase 2.20 — Code Review & Refactor (из Code Review 2026-04-30) ⬜

**Цель:** Устранить технический долг, dead code, HashMap clone overhead, улучшить типизацию, разбить монолитные файлы.

#### 🔴 HIGH PRIORITY — Критические проблемы

| # | Задача | Файл(ы) | Строки | Сложность |
|---|--------|---------|--------|-----------|
| 1 | **Удалить 8 dead SemaError variants** + `collect_identifiers_stmts` (70 строк дубликата) | `sema.rs` | 69–77, 535–606 | Easy |
| 2 | **HashMap clone → `&HashMap` в LowerCtx** | `lower.rs` | 524–531, 652–659 | Medium |
| 3 | **Разбить `lower.rs` (2265 строк) на модули** | `lower.rs` → `lower/` | весь файл | Hard |
| 4 | **Исправить `parser.expect()` — `std::mem::discriminant`** | `parser.rs` | 76, 102 | Medium |
| 5 | **Улучшить `clif_type()` — не всё I64** | `lower.rs` | 697–713 | Medium |
| 6 | **Убрать магический threshold `10_000_000_000`** | `lower.rs` | 1631, 1659 | Easy |

#### 🟡 MEDIUM PRIORITY — Качество кода

| # | Задача | Файл(ы) | Сложность |
|---|--------|---------|-----------|
| 7 | Удалить `append_str_or_int` и `print_str_or_int` (#[allow(dead_code)]) | `lower.rs` | Easy |
| 8 | Удалить `pylang-cli/src/lib.rs` `add()` и `pylang-std` placeholder | `lib.rs` | Easy |
| 9 | Удалить неиспользуемый `Lexer.errors` + `#[allow(dead_code)]` | `lexer.rs` | Easy |
| 10 | Исправить clippy: `LoopContext` более приватный чем `loop_stack` | `lower.rs` | Easy |
| 11 | Исправить `lower_if` — добавить обработку `elif` | `lower.rs` | Medium |
| 12 | Переименовать поля `_name`/`_ty` → без `_` | `lower.rs` | Easy |
| 13 | Удалить дубликат `recv_string` (идентичен `recv`) | `lower.rs` | Easy |
| 14 | Заменить `ast_type_to_clif` (бесполезная обёртка) | `lower.rs` | Easy |
| 15 | Удалить `_main_fn` неиспользуемую переменную | `emit.rs` | Easy |
| 16 | `resolve_method_name` O(N×M) → кэшировать | `lower.rs` | Medium |
| 17 | `extract_int_from_expr` → возвращать `Result` вместо 0 | `lower.rs` | Easy |

#### 🟢 LOW PRIORITY — Стандартизация

| # | Задача | Сложность |
|---|--------|-----------|
| 18 | `has_return_val` — использовать `f.ret` как primary source | Easy |
| 19 | `get_field_offset` — убрать magic names (`_data`, `_len`...) | Easy |
| 20 | `parse_sub_expr` — не создавать новый Parser для f-strings | Medium |
| 21 | `resolve_dot_field_offset` → `Result<i64, String>` | Easy |
| 22 | `lower_fn_inner` — убрать 13 параметров через `LowerPass` struct | Medium |

**Чеклист:**
- [ ] CLI `add()` функция удалена
- [ ] 8 dead SemaError variants удалены
- [ ] `collect_identifiers_stmts` удалён
- [ ] `append_str_or_int` и `print_str_or_int` удалены
- [ ] Lexer.errors удалён или реализован error recovery
- [ ] HashMap clone исправлен (LowerCtx с `&HashMap`)
- [ ] `lower.rs` разбит на модули
- [ ] `parser.expect()` исправлен (не `discriminant`)
- [ ] `clif_type()` различает int/bool/string
- [ ] Магический threshold удалён
- [ ] `lower_if` обрабатывает `elif`
- [ ] `pylang-std` наполнен или удалён
- [ ] Clippy: 0 warnings
- [ ] Тесты: >= 52 passed
- [ ] ELF smoke test: print(42) работает

#### Чеклист — Full Code Review Audit (2026-04-30)

- [x] 27 проблем задокументированы в PLAN.md
- [x] Приоритеты расставлены (🔴 8 critical, 🟡 13 medium, 🟢 5 low)
- [ ] Dead code удалён (8 SemaError + 2 функции + CLI stub + std placeholder)
- [ ] HashMap clone исправлен
- [ ] lower.rs разбит
- [ ] Типизация улучшена
- [ ] Clippy 0 warnings
- [ ] 52 tests pass
...
45. ✅ Never Create Internal CFG Blocks in Loops — не создавать brif/seal внутри цикла
46. ✅ collect_string_vars Must Scan Nested Control Flow — рекурсивный обход вложенных блоков
47. ✅ Use Type Annotations for Variable Type Detection — проверять l.ty/a.ty
48. ✅ No Magic Heuristics for Type Discrimination — использовать типы, а не магические константы
49. ✅ No Dead Code in Repository — удалять неиспользуемый код
50. ✅ Monolithic Files Must Be Split — файлы >1500 строк разбивать на модули
51. ✅ No Duplicate Lowering for Same Builtin — ровно один lowering path
52. ✅ Parser expect() Must Use Full Comparison — не std::mem::discriminant
...
### 45. Never Create Internal CFG Blocks in Loops (КРИТИЧНО)

**Правило:** Функция, вызываемая внутри цикла (for/while/map), НЕ должна создавать собственные Cranelift блоки с `brif`/`jump`/`seal_block`. После возврата такой функции builder остаётся в sealed merge_block, и последующие итерации цикла добавляют инструкции в уже запечатанный блок → порча CFG → segfault или verifier error.

**Пример ошибки:**
```rust
// ❌ НЕПРАВИЛЬНО — append_str_or_int создаёт brif/str_block/int_block/merge_block
// и seal'ит merge_block. Вызывается в цикле lower_fstring → CFG сломан.
fn lower_fstring(lctx, parts) {
    for part in parts {
        match part {
            Expr(e) => {
                let val = lower_expr(e, lctx)?;
                let buf = ...;
                let written = append_str_or_int(lctx, buf, val)?;  // ЛОМАЕТ CFG!
                offset = lctx.builder.ins().iadd(offset, written); // инструкция в sealed block
            }
        }
    }
}
...
**Правильно:**
```rust
// ✅ Использовать прямые runtime вызовы без ветвления внутри цикла
fn lower_fstring(lctx, parts) {
    for part in parts {
        match part {
            Expr(e) => {
                let val = lower_expr(e, lctx)?;
                let buf = ...;
                // Тип известен — используем конкретную runtime функцию
                let written = if is_string_var(e) {
...
**Где это ломается:**
- `append_str_or_int` / `print_str_or_int` — создают str_block/int_block/merge_block внутри цикла
- Любая функция с `lctx.create_block()` + `brif` + `seal_block`, вызываемая в теле цикла

**Альтернативы:**
1. Вынести ветвление в runtime функцию (как `int_to_str`/`str_copy` в lib.rs)
2. Определить тип на этапе компиляции (через `string_vars`/`param_types`/`let_ty`)
3. Использовать `#[allow(dead_code)]` для вспомогательной функции и не вызывать её в циклах

### 46. collect_string_vars Must Scan Nested Control Flow

**Правило:** `collect_string_vars` должен рекурсивно обходить тела `if`/`elif`/`else`/`while`/`for`/`loop`/`match` для поиска строковых переменных. Переменная `content: str = ...` внутри `while`/`if` блока должна попадать в `string_vars`.

**Пример ошибки:**
```rust
// ❌ НЕПРАВИЛЬНО — сканирует только top-level statements
fn collect_string_vars(body: &[Stmt], ...) -> HashSet<String> {
    for stmt in body {
        match stmt {
            Stmt::Let(l) => { if is_string { sv.insert(l.name); } }
            _ => {}  // If/While/For пропущены!
        }
    }
}
```
...
**Правильно:**
```rust
// ✅ Рекурсивно заходит в control flow
fn collect_string_vars(body: &[Stmt], ...) -> HashSet<String> {
    for stmt in body {
        match stmt {
            Stmt::Let(l) => { if is_string { sv.insert(l.name); } }
            Stmt::If(i) => {
                sv.extend(collect_string_vars(&i.then, ...));
                if let Some(else_) = &i.else_ {
                    sv.extend(collect_string_vars(else_, ...));
                }
                for elif in &i.elif {
                    sv.extend(collect_string_vars(&elif.body, ...));
                }
            }
            Stmt::While(w) => { sv.extend(collect_string_vars(&w.body, ...)); }
            Stmt::For(f) => { sv.extend(collect_string_vars(&f.body, ...)); }
            Stmt::Loop(l) => { sv.extend(collect_string_vars(&l.body, ...)); }
            Stmt::Match(m) => {
                for arm in &m.arms {
                    sv.extend(collect_string_vars(&arm.body, ...));
                }
            }
            _ => {}
        }
...
### 47. Use Type Annotations for Variable Type Detection

**Правило:** При определении типа переменной использовать её type annotation (`l.ty`, `a.ty`) когда `is_expr_known_string` или `var_is_string` не могут определить тип (например, для вызовов через `dict_call`, `Expr::Index`, или других не-Ident выражений).

**Пример ошибки:**
```rust
// ❌ НЕПРАВИЛЬНО — content: str = self.routers["/health"]() не распознан как строка
// is_expr_known_string возвращает false для Call с не-Ident func
Stmt::Let(l) => {
    if is_expr_known_string(&l.val, ...) {  // false — Expr::Call с Expr::Index func
...
**Правильно:**
```rust
// ✅ Проверяем type annotation в Let/LetMut/AnnAssign
...
Stmt::Let(l) => {
    if let_ty_is_string(&l.ty) || is_expr_known_string(&l.val, ...) {
        sv.insert(l.name.clone());
    }
}
```

### 48. No Magic Heuristics for Type Discrimination

**Правило:** НЕ использовать магические константы/эвристики для различения типов во время компиляции (например, `value > 10_000_000_000` → pointer). Если компилятор знает тип — использовать эту информацию. Если нет — изменить компилятор чтобы знал.

**Пример ошибки:**
```rust
// ❌ НЕПРАВИЛЬНО — хрупкая эвристика: int > 10B → pointer
let is_ptr = builder.ins().icmp(IntCC::UnsignedGreaterThan, val, threshold);
if is_ptr { ... } else { ... }
```

**Правильно:**
```rust
// ✅ Использовать информацию о типах из sema
let is_string = lctx.string_vars.contains(name) || lctx.param_types.get(name) == Some(&AstType::String);
if is_string { str_copy(lctx, buf, val) } else { int_to_str(lctx, buf, val) }
```

### 49. No Dead Code in Repository

**Правило:** Код, который никогда не вызывается и не может быть вызван — должен быть удалён. Использовать `cargo clippy` и поиск `rg "function_name"` для проверки всех call sites.

**Чеклист проверки:**
```bash
# Проверить unused/covered warnings
cargo clippy 2>&1 | grep -E "unused|dead_code|never constructed"

# Проверить что каждая SemaError variant используется
rg "SemaError::" sema.rs

# Проверить что каждый Expr/Stmt обрабатывается в lowering
# (сравнить ast.rs enum варианты с match arms в lower.rs)
```

### 50. Monolithic Files Must Be Split

**Правило:** Файл > 1000 строк должен быть кандидатом на разбивку. Файл > 1500 строк — критический кандидат. Текущие монолиты:
- `lower.rs` — 2265 строк (38% кодовой базы)
- `sema.rs` — 1677 строк (28% кодовой базы)

Разбивать по функциональности (control flow, builtins, expressions, classes) а не по произвольным границам.

### 51. No Duplicate Lowering for Same Builtin

**Правило:** Каждая builtin-функция должна иметь ровно один lowering path. Дубликаты (`recv` / `recv_string`) — недопустимы.

**Пример ошибки:**
```rust
// ❌ НЕПРАВИЛЬНО — recv и recv_string идентичны
"recv" => { /* lowering A */ }
"recv_string" => { /* lowering A — идентично */ }
```

**Правильно:**
```rust
// ✅ Одна реализация
"recv" | "recv_string" => { /* lowering A */ }
```

### 52. Parser expect() Must Use Full Comparison

**Правило:** `std::mem::discriminant` сравнивает только variant enum, но не значения внутри variant. `TokenKind::Ident("foo")` == `TokenKind::Ident("bar")` с discriminant. Для токенов с данными (Int, Float, Str, Ident, FString) это ломает парсинг.

**Пример ошибки:**
```rust
// ❌ НЕПРАВИЛЬНО — пропускает любой Ident
fn expect(&mut self, kind: TokenKind) -> Result<Spanned<TokenKind>, ParseError> {
    if std::mem::discriminant(&t.value) == std::mem::discriminant(&kind) {
        // OK для TokenKind::Def, но для TokenKind::Ident(_) сравнивает только discriminant
        return self.bump();
    }
    // Never reached for TokenKind::Ident(String::new())
}
```

**Правильно:**
```rust
// ✅ Добавить дженерик вариант или сравнивать с любым Ident
fn expect(&mut self, kind: TokenKind) -> Result<Spanned<TokenKind>, ParseError> {
    let matches = match (&kind, &t.value) {
